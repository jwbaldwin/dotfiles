...
Run this exact command shape:

```bash
/Users/jbaldwin/.local/bin/plannotator review --git [optional-pr-url]
```

Always include `--git`, especially in JJ workspaces. Do not run bare
`plannotator review`. Plannotator's native JJ diff path can emit hunk line
metadata that crashes the browser diff renderer with `Expected node, not`.
